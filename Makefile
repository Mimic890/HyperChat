...
if storage_type == 'local':
    ...
    with open(storage_compose, 'w') as f:
        ...
        f.write('# Replaces named volumes with direct bind-mounts so no Docker volumes are created\n')
        f.write('services:\n')
        f.write(f'  postgres:\n    volumes:\n      - {data_path}/postgres:/var/lib/postgresql/data\n      - ./postgres/init-databases.sh:/docker-entrypoint-initdb.d/init-databases.sh:ro\n')
        f.write(f'  redis:\n    volumes:\n      - {data_path}/redis:/data\n')
        f.write(f'  synapse-init:\n    volumes:\n      - {data_path}/synapse:/data\n')
        f.write(f'  synapse:\n    volumes:\n      - ./synapse/homeserver.yaml:/config/homeserver.yaml:ro\n      - ./synapse/log.config:/config/log.config:ro\n      - {data_path}/synapse:/data\n      - ./bridges/telegram:/bridges/telegram:ro\n      - ./bridges/whatsapp:/bridges/whatsapp:ro\n      - ./bridges/discord:/bridges/discord:ro\n      - ./bridges/signal:/bridges/signal:ro\n')
    ok(storage_compose)
...
if en('MAS'):
    ...
    lines.append(sec_headers)
    lines.append(f'')
    lines.append(f'    # MAS handles its own CORS — just proxy')
    lines.append(f'    handle {{')
    lines.append(f'        reverse_proxy localhost:{port_mas}')
    lines.append(f'    }}')
    lines.append(f'}}')
