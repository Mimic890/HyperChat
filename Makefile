...
info('\nWriting .env...\n')
_generated_keys = {'COMPOSE_FILE','COMPOSE_PROFILES','BIND_ADDR',
    'TRAEFIK_ENTRYPOINTS','TRAEFIK_TLS','ELEMENT_CALL_URL',
    'HOST_MATRIX','HOST_ELEMENT','HOST_CINNY','HOST_LIVEKIT','HOST_STICKERS','HOST_MAS'}
with open(ENV_FILE) as src: original_lines = src.readlines()
cleaned = []
for _l in original_lines:
    _m = re.match(r'^([A-Z_][A-Z0-9_]*)=', _l)
    if _m and _m.group(1) in _generated_keys:
        continue
    if _l.strip() == '# Generated by make build — do not edit':
        continue
    cleaned.append(_l)
...
lines.append(f'')
same_domain = (host_element == host_matrix)  # Element and Matrix on same domain
lines.append(f'# Matrix homeserver' + (' + Element Web' if same_domain and en('ELEMENT') else ''))
lines.append(f'{host_matrix} {{')
lines.append(f'    encode zstd gzip')
lines.append(sec_headers)
lines.append(f'')
# .well-known/matrix/server (federation) — always emit when same domain or matrix-only
lines.append(f'    handle /.well-known/matrix/server {{')
lines.append(f'        respond `{wk_server}` 200')
lines.append(f'    }}')
# .well-known/matrix/client — with MSC2965 if MAS enabled
if en('MAS'):
    _wk_mas = '{' + f'"m.homeserver":{{"base_url":"https://{host_matrix}"}},"org.matrix.msc2965.authentication":{{"issuer":"https://{host_mas}/"}}' + '}'
    lines.append(f'    handle /.well-known/matrix/client {{')
    lines.append(f'        header Access-Control-Allow-Origin *')
    lines.append(f'        respond `{_wk_mas}` 200')
    lines.append(f'    }}')
else:
    lines.append(f'    handle /.well-known/matrix/client {{')
    lines.append(f'        header Access-Control-Allow-Origin *')
...
if en('MAS'):
    ...
    lines.append(f'')
# Matrix API routes
lines.append(f'    # Matrix API → Synapse')
lines.append(f'    handle /_matrix/* {{')
lines.append(f'        reverse_proxy localhost:{port_synapse} {{')
lines.append(f'            flush_interval -1')
lines.append(f'        }}')
lines.append(f'    }}')
lines.append(f'    handle /_synapse/* {{')
lines.append(f'        reverse_proxy localhost:{port_synapse} {{')
lines.append(f'            flush_interval -1')
lines.append(f'        }}')
lines.append(f'    }}')
if same_domain and en('ELEMENT'):
    lines.append(f'')
    lines.append(f'    # Element Web → everything else')
...
lines.append(f'}}')
lines.append(f'')
# Element block — only if Element is on a different domain than Matrix
if en('ELEMENT') and host_element != host_matrix:
...
if en('MAS'):
    ...
    lines.append(sec_headers)
    lines.append(f'')
    lines.append(f'    # CORS for Element Admin (admin-beta.element.dev)')
    lines.append(f'    @cors_preflight {{')
    lines.append(f'        method OPTIONS')
    lines.append(f'        header Origin https://admin-beta.element.dev')
    lines.append(f'    }}')
    lines.append(f'    handle @cors_preflight {{')
    lines.append(f'        header Access-Control-Allow-Origin "https://admin-beta.element.dev"')
    lines.append(f'        header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, OPTIONS"')
    lines.append(f'        header Access-Control-Allow-Headers "Authorization, Content-Type"')
    lines.append(f'        header Access-Control-Max-Age "86400"')
    lines.append(f'        respond "" 204')
    lines.append(f'    }}')
    lines.append(f'')
    lines.append(f'    handle {{')
    lines.append(f'        header Access-Control-Allow-Origin "https://admin-beta.element.dev"')
