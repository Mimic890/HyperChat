...
mode = int(get('DEPLOY_MODE', '1'))
domain = get('DOMAIN', 'localhost')
port_synapse  = get('PORT_SYNAPSE',  '28008')
port_element  = get('PORT_ELEMENT',  '28080')
port_cinny    = get('PORT_CINNY',    '28081')
port_admin    = get('PORT_ADMIN',    '28082')
port_livekit  = get('PORT_LIVEKIT',  '27880')
port_stickers = get('PORT_STICKERS', '28090')
en = lambda k: get(f'ENABLE_{k}', 'false').lower() == 'true'
if mode in (2, 4):
...
wk_server = '{"m.server": "matrix.' + domain + ':443"}'
wk_client = '{"m.homeserver":{"base_url":"https://matrix.' + domain + '"}}'
lines = []
lines.append(f'# ── HyperChat — generated by: make caddy ──────────────────────────────────')
lines.append(f'')
lines.append(f'# Matrix homeserver')
lines.append(f'matrix.{domain} {{')
...
if en('ELEMENT'):
    ...
    lines.append(f'# .well-known for Matrix federation')
    lines.append(f'{domain} {{')
    lines.append(f'    handle /.well-known/matrix/server {{')
    lines.append(f'        respond `{wk_server}` 200')
    lines.append(f'    }}')
...
needed = [f'matrix.{domain}']
if en('ELEMENT'): needed.append(domain)
if en('CINNY'):   needed.append(f'cinny.{domain}')
if en('VOIP'):    needed.append(f'livekit.{domain}')
if en('STICKERS'): needed.append(f'stickers.{domain}')
for r in needed:
